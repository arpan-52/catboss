...
def sumthreshold_gpu(amp, existing_flags, baseline_info, combinations=None, sigma_factor=6.0, rho=1.5, diagnostic_plots=False, stream=None, precalculated_thresholds=None):
    ...
    if precalculated_thresholds is not None:
        ...
        print("Calculating channel-based statistics on CPU (respecting existing flags)...")
...
def get_memory_info():
    ...
    try:
        ...
        print(f"GPU memory: {gpu_mem/1e9:.2f} GB (usable: {gpu_usable_mem/1e9:.2f} GB)")
    except Exception as e:
...
def calculate_baseline_batch_size(baseline_data, sample_bl, options, gpu_usable_mem, system_usable_mem):
    ...
    gpu_mem_per_bl = time_samples * freq_channels * 4 * 3 * corr_factor  # 4 bytes for float32
    ...
    if options.get('apply_flags', False):
        system_mem_per_bl += time_samples * freq_channels * n_corr  # 1 byte for bool flags
...
def hunt_ms(ms_file, options):
    ...
    try:
        ...
        total_system_mem = system_mem.total
        available_system_mem = system_mem.available
        system_usable_mem = available_system_mem * options.get('max_memory_usage', 0.8)  # Use configurable % of available memory
        print(f"System memory: {total_system_mem/1e9:.2f} GB (available: {available_system_mem/1e9:.2f} GB, using: {system_usable_mem/1e9:.2f} GB)")
    except Exception as e:
        print(f"Warning: Could not determine system memory, assuming 16GB: {str(e)}")
        system_usable_mem = 16 * 1024 * 1024 * 1024 * 0.8  # Assume 16GB, use 80%
    ...
    for field_id in field_ids:
        ...
        memory_per_bl_gpu = time_samples * freq_channels * 4 * 3  # 4 bytes for float32, 3 for data+flags+thresholds
        
        # System memory requirements
        memory_per_bl_cpu = time_samples * freq_channels * 8 * n_corr  # 8 bytes for complex64 data + flags
        
        # Additional memory if plots are being generated
        if options['diagnostic_plots']:
            memory_per_bl_cpu += time_samples * freq_channels * 4 * 3  # Additional copies for plotting
        
        print(f"Typical baseline has {time_samples} time samples × {freq_channels} frequency channels")
        print(f"Estimated memory per baseline: {memory_per_bl_gpu / (1024*1024):.2f} MB (GPU), {memory_per_bl_cpu / (1024*1024):.2f} MB (CPU)")
        ...
        print(f"Can process approximately {bl_per_batch} complete baselines in memory at once")
        ...
        if memory_per_bl_gpu > usable_mem:
            ...
            print(f"Single baseline requires chunking! Splitting into {chunk_factor} chunks")
                    ...
                    if options['apply_flags']:
                        ...
                        try:
                            ...
                            for bl in batch_results.keys():
                                processed_bl_clauses.append(f"(ANTENNA1={bl[0]} AND ANTENNA2={bl[1]})")
                                ...
                                for row_idx, (a1, a2) in enumerate(zip(ant1, ant2)):
                                    ...
                                    if bl in batch_results:
                                        ...
                                        if row_idx < bl_flags.shape[0]:
                                            new_flags[row_idx] = np.logical_or(new_flags[row_idx], bl_flags[row_idx])
                                
                                # Convert back to dask array with same chunking as original
                                import dask.array as da
                                new_flags_dask = da.from_array(new_flags, chunks=orig_flags.chunks)
                            ...
                            if updated_datasets:
                                try:
                                    print(f"Writing flags for batch {i//bl_per_batch + 1}...")
                                    write_back = xds_to_table(updated_datasets, ms_file, ["FLAG"])
                                    dask.compute(write_back)
                                    print(f"Successfully wrote flag updates for batch {i//bl_per_batch + 1}")
                                except Exception as e:
            ...
            for bl_idx, bl in enumerate(tqdm(baselines, desc="Baselines")):
                ...
                try:
                    ...
                    if chunk_factor > 1:
                        ...
                        for (start_idx, end_idx, chunk_info), result in zip(chunk_infos, chunk_results):
                            ...
                            if chunk_flags_result is not None:
                                chunk_flags.append((start_idx, end_idx, chunk_flags_result))
                        
                        # Combine chunk flags if needed
                        if chunk_flags:
                            # Initialize flags array for full baseline
                            all_chunk_flags = np.zeros((bl_data.shape[0], bl_data.shape[1], bl_data.shape[2]), dtype=bool)
                        ...
                        all_chunk_flags, baseline_existing, baseline_new = process_baseline_async(
                    ...
                    if options['apply_flags'] and all_chunk_flags is not None:
                        try:
                            ...
                            if orig_ds_list:
                                ...
                                combined_flags = np.logical_or(orig_flags_array, all_chunk_flags)
                            ...
                            print(f"Error preparing flag updates for baseline {bl}: {str(e)}")
                            import traceback
                            traceback.print_exc()
...
def process_baseline_async(bl_data, bl, field_id, corr_to_process, options, freq_axis, total_flagged, total_visibilities, stream=None, chunk_info="", output_dir='outputs'):
    ...
    if len(corr_to_process) == 1:
        ...
        if np.all(existing_flags):
            if options['verbose']:
                print(f"All data is already flagged for baseline {bl} corr {corr_idx} {chunk_info}. Skipping.")
            if options['apply_flags']:
        ...
        if np.all(existing_flags_combined):
            if options['verbose']:
                print(f"All data is already flagged for baseline {bl} combined corrs {chunk_info}. Skipping.")
            if options['apply_flags']:
...
def process_baselines_batch_gpu(baseline_data, field_id, corr_to_process, options, freq_axis):
    ...
    for bl in baselines:
        ...
        if len(corr_to_process) == 2 and corr_to_process[0] == 0 and corr_to_process[-1] == bl_data.DATA.shape[2] - 1:
            ...
            threshold_arrays[bl] = {
                'base_thresholds': channel_medians + options['sigma_factor'] * channel_stds,
                'vis_amp': vis_amp,
            ...
            for corr_idx in corr_to_process:
                ...
                baseline_corr_key = (bl[0], bl[1], corr_idx)  # Use a flat tuple to avoid nested tuples
            ...
            if mode == 'combined':
                ...
                if bl not in results:
                    ...
                    for corr_idx in corr_to_process:
                        ...
                        # To:
                        full_flags[:, :, corr_idx] = np.logical_or(full_flags[:, :, corr_idx], combined_flags_corr)
                ...
                results[orig_bl][:, :, corr_idx] = np.logical_or(results[orig_bl][:, :, corr_idx], combined_flags_corr)
...
def generate_diagnostic_plot(vis_amp, combined_flags, existing_flags, bl, field_id, corr_idx, freq_axis, output_dir):
    ...
    try:
        print(f"Generating plot for bl {bl[0]}-{bl[1]} pol {corr_str}, shape: {vis_amp.shape}")
        
        # Check if the array is too large to plot efficiently
        total_pixels = vis_amp.size
        if total_pixels > 1000000:  # 1 million pixels
            print(f"Warning: Large data array ({total_pixels} pixels) may consume significant memory")
        ...
        plt.imshow(vis_amp, aspect='auto', origin='lower', cmap='viridis',
        ...
        percent_new = 100 * (np.sum(combined_flags) - np.sum(existing_flags)) / combined_flags.size
