...
def calculate_baseline_batch_size(
    ...
    # CRITICAL: Add 30% safety margin to prevent OOM kills
    # Account for: dask overhead, Python overhead, OS processes, intermediate arrays
    bl_batch_size = max(1, int(bl_batch_size * 0.7))
    ...
    logger.info(f"  - Selected batch size (with 30% safety margin): {bl_batch_size} baselines")
...
def process_single_field(
    ...
    # Process all baselines (no pre-filter)
    # If already flagged, processing is fast anyway - no benefit to checking first
    valid_baselines = baselines
    logger.info(f"\nProcessing all {len(valid_baselines)} baselines")
